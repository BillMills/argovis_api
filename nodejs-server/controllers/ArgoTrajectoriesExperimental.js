...
module.exports.findArgoTrajectory = function findArgoTrajectory (req, res, next, id, startDate, endDate, polygon, box, center, radius, metadata, platform, compression, data, batchmeta) {

  apihits.apihits.create({metadata: req.openapi.openApiRoute, query: req.query, isWeb: req.headers.origin === 'https://argovis.colorado.edu', avhTelemetry: req.headers.hasOwnProperty('x-avh-telemetry') ? req.headers['x-avh-telemetry'] : null})

  ArgoTrajectoriesExperimental.findArgoTrajectory(res, id, startDate, endDate, polygon, box, center, radius, metadata, platform, compression, data, batchmeta)
    .then(
